...
// 비선형 자료구조 - 힙
// 완전 이진트리 형태이기 때문에 데이터의 연속성이 보장되어 배열자료구조 형태인 ArrayList로 최대 힙 구현
...
class MaxHeap {
    ...
    public void insert(int data) {
        ...
        while (cur > 1 && heap.get(cur / 2) < heap.get(cur)) {
            ...
            // 비교한 노드보다 더 상위 노드와의 비교를 위해 cur을 2로 나눈다.
            cur /= 2;
        }
    }
...
public class Heap_Lecture2 {
    public static void main(String[] args) {
        ...
        maxHeap.insert(40);
        maxHeap.insert(10);
        maxHeap.printTree(); // 40 30 10 출력
        maxHeap.insert(50);
        maxHeap.insert(60);
        maxHeap.insert(70);
        maxHeap.printTree(); // 70 50 60 30 40 10 출력
        maxHeap.insert(20);
        maxHeap.printTree(); // 70 50 60 30 40 10 20 출력
        maxHeap.insert(30);
        maxHeap.printTree(); // 70 50 60 30 40 10 20 30

        System.out.println();
        System.out.println("** 데이터 삭제 **");
        System.out.println("삭제 : " + maxHeap.delete()); // 최상위 데이터 70 삭제 및 가장 및 데이터 30을 올린 후 최소 힙 기준에 맞춰 조정.
        maxHeap.printTree(); // 60 50 30 30 40 10 20
        System.out.println("삭제 : " + maxHeap.delete()); // 최상위 데이터 60 삭제 및 가장 및 데이터 20을 올린 후 최소 힙 기준에 맞춰 조정.
        maxHeap.printTree(); // 50 40 30 30 20 10
        System.out.println("삭제 : " + maxHeap.delete()); // 최상위 데이터 50 삭제 및 가장 및 데이터 10을 올린 후 최소 힙 기준에 맞춰 조정.
